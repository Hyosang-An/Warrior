...
{
	NodeName = TEXT("Native Rotate to Target Actor");
...
void UBTTask_RotateToFaceTarget::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	else
	{
		...
		const FRotator NewRotation = FMath::RInterpTo(Memory->OwningPawn->GetActorRotation(), LookAtRot, DeltaSeconds, RotationInterpSpeed);
		//const FRotator NewRotation = FMath::RInterpConstantTo(Memory->OwningPawn->GetActorRotation(), LookAtRot, DeltaSeconds, 360.f);

		Memory->OwningPawn->SetActorRotation(NewRotation);
	}
}
...
bool UBTTask_RotateToFaceTarget::HasReachedAnglePrecision(APawn* QueryPawn, AActor* TargetActor) const
{
	...
	const FVector TargetDirection = (TargetActor->GetActorLocation() - QueryPawn->GetActorLocation()).GetSafeNormal();

	return FVector::DotProduct(OwnerForward, TargetDirection) >= FMath::Cos(FMath::DegreesToRadians(AnglePrecision));
}